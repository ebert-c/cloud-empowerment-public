...
class PathologyForm extends StatefulWidget {
  ...
  PathologyForm({Key? key, required this.session}) : super(key: key);

  @override
  _PathologyFormState createState() => _PathologyFormState();
}
...
class _PathologyFormState extends State<PathologyForm> {
  ...
  void initState() {
    super.initState();
    values = widget.session.encounter?.encounterData[COMPONENT_NAMES.PATHOLOGY] ?? values;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        appBar: AppBar(title: Text('Pathology')),
                      ...
                      Center(
                        child: ElevatedButton(
                          onPressed: () {
                            ...
                            widget.session.encounter?.addComponentData(COMPONENT_NAMES.PATHOLOGY, values);
