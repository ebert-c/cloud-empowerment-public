...
class SurgeryForm extends StatefulWidget {
  ...
  SurgeryForm({Key? key, required this.session}) : super(key: key);

  @override
  _SurgeryFormState createState() => _SurgeryFormState();
}
...
class _SurgeryFormState extends State<SurgeryForm> {
  ...
  void initState() {
    super.initState();
    values = widget.session.encounter?.encounterData[COMPONENT_NAMES.SURGERY] ?? values;
                      ...
                      Center(
                        child: ElevatedButton(
                          onPressed: () {
                            ...
                            widget.session.encounter?.addComponentData(COMPONENT_NAMES.SURGERY, values);
